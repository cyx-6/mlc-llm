...
def encoding_func(nbit: int, storage_nbit: int, group_size: int, dtype: str = "float32"):
    def te_encode_sym(weight: te.Tensor):
        ...
        if weight.ndim == 2:
            ...
            cur_group_size = weight.shape[2] if group_size == -1 else group_size
            scale_min_shape = (weight.shape[0], tir.ceildiv(weight.shape[2], cur_group_size), weight.shape[1])
            k = te.reduce_axis((0, cur_group_size), name="k")
            max_abs_value = te.compute(
                shape=scale_min_shape,
                fcompute=lambda batch, group, i: te.max(
                    te.abs(
                        tir.if_then_else(
                            group * cur_group_size + k < weight.shape[2],
                            weight[batch, i, group * cur_group_size + k],
                            tir.const(0, dtype=weight.dtype),
                        )
                    ),
                    axis=k,
            ...
            def f_scale_weight(b, i, j):
                w_scaled = tir.round(tir.Cast(dtype, weight[b, i, j]) / scale[b, j // cur_group_size, i])
                w_scaled = T.min(
                    T.max(w_scaled, tir.const(-max_int_value - 1, dtype)),
                    tir.const(max_int_value, dtype),
                ).astype(storage_dtype)
            ...
            if n_float_per_int == 1:
                w_gathered = te.compute(
                    shape=(weight.shape[0], weight.shape[2], n_i32),
                    fcompute=lambda b, j, i: f_scale_weight(b, i, j),
                    name="w_gathered",
                )
            else:
                k = te.reduce_axis((0, n_float_per_int), name="k")
                ...
                w_gathered = te.compute(
                    shape=(weight.shape[0], weight.shape[2], n_i32),
                    fcompute=lambda b, j, i: reducer(
                        tir.if_then_else(
                            i * n_float_per_int + k < weight.shape[1],
                            f_scale_weight(b, i * n_float_per_int + k, j)
                            << (k.astype(storage_dtype) * tir.const(nbit, storage_dtype)),
                            tir.const(0, storage_dtype),
                        ),
                        axis=k,
...
def decoding_func(nbit: int, storage_nbit: int, group_size: int):
    def te_decode_sym(data: te.Tensor, scale: te.Tensor):
        if data.ndim == 2:
            assert scale.ndim == 2
            n_float_per_int = storage_nbit // nbit
            ...
            def f_decode_sym(b, i, j):
                if n_float_per_int == 1:
                    data_float = tir.Cast("float16", data[b, i, j])
                else:
                    f_convert = tir_utils._tir_packed_int_to_int_to_float(storage_nbit)
                    data_float = f_convert(
                        nbit, data[b, i, j // n_float_per_int], j % n_float_per_int, dtype="float16"
                    )

                scale_float = scale[b, i // cur_group_size, j]
                return data_float * scale_float

            shape = (data.shape[0], data.shape[1], data.shape[2] * n_float_per_int)
            w = te.compute(shape=shape, fcompute=f_decode_sym, name="decode")
            # Dummy transpose for FuseDecodeTranspose
            return topi.transpose(w)
